...
def generate_timestep_samples(
    ...
    """
    Generate samples at different timesteps using NON-UNIFORM timesteps.
    ...
    print(f"Generating samples using {len(timesteps)} non-uniform timesteps...")
    ...
    with torch.no_grad():
        for i, current_t in enumerate(timesteps):
            ...
            if i < len(timesteps) - 1:
                ...
                # Midpoint method for this non-uniform interval
...
def plot_angular_difference(analysis_dict, output_path):
    ...
    fig.suptitle('Velocity Field Analysis', fontsize=14)
...
# def generate_timestep_list():
#     """Generate the specified timestep list."""
#     timesteps = []
    
#     # 0.00 to 0.10 with step 0.01
#     timesteps.extend(np.arange(0.00, 0.21, 0.01))
    
#     # 0.3, 0.5, 0.7, 0.9
#     timesteps.extend([0.3,0.4 ,0.5,0.6 , 0.7, 0.8,0.9])
    
#     # 0.91 to 1.00 with step 0.01
#     timesteps.extend(np.arange(0.91, 1.01, 0.01))
    
#     # Remove duplicates and sort
#     timesteps = sorted(list(set(np.round(timesteps, 3))))
...
def main():
    parser = argparse.ArgumentParser("Generate samples at different timesteps")
    parser.add_argument("--checkpoint", default = "checkpoint-1799.pth", help="Path to checkpoint")
    parser.add_argument("--num_samples", type=int, default=16, help="Number of samples to generate")
    parser.add_argument("--output_dir", default="./timestep_samples", help="Output directory")
    ...
    timesteps = generate_timestep_list()
    # Convert tensor to list of floats
    if isinstance(timesteps, torch.Tensor):
        timesteps = timesteps.tolist()
    print(f"Generating samples for {len(timesteps)} timesteps:")
    print(f"Timesteps: {[f'{t:.2f}' for t in timesteps]}")
