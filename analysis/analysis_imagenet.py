...
def plot_multi_batch_analysis(analysis, output_dir, dataset_name):
    ...
    fig.suptitle(f"Velocity Field Analysis - {dataset_name} ({num_batches} batches, {total_samples} total samples)", fontsize=14)
    ...
    with open(summary_path, 'w') as f:
        f.write(f"=== Multi-Batch Velocity Field Analysis - {dataset_name} ===\n")
...
def main():
    parser = argparse.ArgumentParser("ImageNet Flow Matching Inference with Multi-Batch Cosine Similarity Analysis")
    parser.add_argument("--checkpoint", required=True, help="Path to checkpoint")
    parser.add_argument("--dataset", choices=["imagenet32", "imagenet64"], default="imagenet64", 
                       help="Dataset/architecture to use (imagenet32 or imagenet64)")
    parser.add_argument("--total_samples", type=int, default=512, 
                       help="Total number of samples to generate (default: 512 for ImageNet)")
    parser.add_argument("--batch_size", type=int, default=16, 
                       help="Batch size for each inference run (default: 16 for ImageNet)")
    parser.add_argument("--output", default=None, help="Output directory for all generated files")
    ...
    if args.output is None:
        args.output = f"./imagenet_analysis_{args.dataset}"
    output_dir = args.output
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    print(f"Output directory: {output_dir}")
    print(f"Dataset: {args.dataset} (image size: {image_size}x{image_size})")
    
    # Initialize model (using the imagenet architecture)
    print(f"Initializing {args.dataset} model...")
    ...
    # Plot analysis
    plot_multi_batch_analysis(analysis, output_dir, args.dataset.upper())
