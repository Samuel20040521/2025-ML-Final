...
class Logger(object):
    ...
    def log(self, field: str, data: Any):
        ...
        if self.use_wandb:
            ...
            self.info(f"{field}: {data.item()}")
        elif isinstance(data, (int, float, str)):
            self.info(f"{field}: {data}")
        else:
            raise ValueError("Only python numerical types and single-element"
                       "tensors are supported for data")

    def log_vector(self, field: str, vector: torch.Tensor, label: str, value: str, title: str):
        if not self.is_main_process() or not self.use_wandb or self.silent:
            return
